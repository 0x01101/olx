...
export async function fetchNotifications(): Promise<Notification[]> {
    const query: string = `
        SELECT id, source, user_id, title, content, created_at
        FROM notifications;
    `;
    return await execQuery(query);
}
...
export async function fetchUsersWhere(propertyName: string, propertyValue: any): Promise<User[]> {
    const query: string = `
        SELECT id, username, email, password, role, created_at
        FROM users
        WHERE ${propertyName} = ?;
...
export async function fetchCategoriesWhere(propertyName: string, propertyValue: any): Promise<Category[]> {
    const query: string = `
        SELECT id, name, created_at, logo_path
        FROM categories
        WHERE ${propertyName} = ?;
...
export async function fetchProductsWhere(propertyName: string, propertyValue: any): Promise<Product[]> {
    const query: string = `
        SELECT products.id,
               products.uuid,
               products.name,
...
export async function fetchBidsWhere(propertyName: string, propertyValue: any): Promise<Bid[]> {
    const query: string = `
        SELECT bids.id,
               bids.amount,
               bids.created_at,
...
export async function fetchTransactionsWhere(propertyName: string, propertyValue: any): Promise<Transaction[]> {
    const query: string = `
        SELECT transactions.id,
               transactions.amount,
               transactions.created_at,
...
export async function fetchNotificationsWhere(propertyName: string, propertyValue: any): Promise<Notification[]> {
    const query: string = `
        SELECT id, source, user_id, title, content, created_at
        FROM notifications
        WHERE ${propertyName} = ?;
